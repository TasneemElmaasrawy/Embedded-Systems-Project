...
int main(void)
{
  ...
  HAL_I2C_Master_Transmit(&hi2c1, 0xD0, year, 2, 10);
    ...
    HAL_I2C_Master_Receive(&hi2c1, 0xD1, day+1, 1, 10);
		...
		HAL_UART_Receive(&huart2,&receive_c2, sizeof(receive_c2),50); 			// receive command from the web interface through wifi
		if (receive_c == 'd' || receive_c2 == 'd' )								//send date and time when recieving 'd'
		{
			if (receive_c == 'd') HAL_UART_Transmit(&huart1,date_time, sizeof(date_time),500);
			
			if (receive_c2 == 'd') HAL_UART_Transmit(&huart2,date_time, sizeof(date_time),500);
			receive_c = '\0';
			receive_c2 = '\0';
		}
